...
**Database compatibility:** PostgreSQL is the committed backend. Both `watchlist/services.py` and `review/services.py` use `sqlalchemy.dialects.postgresql.insert().on_conflict_do_update(...)`, and the watchlist model uses a PostgreSQL enum. Supporting another database requires code and schema changes.
